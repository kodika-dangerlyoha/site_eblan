let games = [
    {
        'id': 0,
        'name': "Call Of Duty: Cold War 2",
        'link': "gameCard.html",
        'steamLink': "",
        'imgH': "img/banners/farmsim.png",
        'imgW': "img/banners/img1.png",
        'treilerSrc': "video/50.MP4",
        'bigBanner': "img/banners/big.png",
        'newPrice': 4599,
        'oldPrice': 5120,
        'editions': [
            {
                'title': "Cold War 2: mamamia",
                'newPrice': 5000,
                'oldPrice': 5500,
                'link': "",
            },
            {
                'title': "Cold War 2: mamamia 2",
                'newPrice': 5200,
                'oldPrice': 5600,
                'link': "",
            },
            {
                'title': "Cold War 2: mamamia 3",
                'newPrice': 5000,
                'oldPrice': 7000,
                'link': "",
            },
        ],
        'description': `
        <h2>Об этой игре</h2> An entirely new experience from the makers of the ‘The Forest’
        <br>
        <br><img src="https://cdn.cloudflare.steamstatic.com/steam/apps/1326470/extras/VirginiaWaterfall.gif?t=1708624856">
        <br>
        <br>Sent to find a missing billionaire on a remote island, you find yourself in a cannibal-infested hellscape. Craft, build, and struggle to survive, alone or with friends, in this terrifying new open-world survival horror simulator.
        <h2 class="bb_tag"><strong>A Survival Horror Simulator</strong></h2>
        <br>Experience complete freedom to tackle the world how you want. You decide what you do, where to go and how best to survive. There are no NPC's barking orders at you or giving you missions you don't want to do. You give the orders, you choose what happens next.
        <h2 class="bb_tag"><strong>Fight Demons</strong></h2>
        <br>Enter a world where nowhere is safe and fight against a range of mutated creatures, some who are almost human like, and others who are like nothing you have ever seen before. Armed with pistols, axes, stun batons and more, protect yourself and those you care for.
        <h2 class="bb_tag"><strong>Build and Craft</strong></h2>
        <br>Feel every interaction; Break sticks to make fires. Use an axe to cut out windows and floors. Build a small cabin, or a sea-side compound, the choice is yours.
        <h2 class="bb_tag"><strong>Changing Seasons</strong></h2>
        <br>Pluck fresh salmon directly from streams in spring and summer. Collect and store meat for the cold winter months. You're not alone on this island, so as winter rolls in and food and resources become scarce you won't be the only one looking for a meal.
        <h2 class="bb_tag"><strong>Co-op Gameplay</strong></h2>
        <br>Survive alone, or with friends. Share items and work together to build defenses. Bring back-up to explore above and below ground.`,
        'systemRequirements': `
          <ul>
            <strong>Минимальные:</strong>
            <br>
            <ul class="bb_ul">
              <li>64-разрядные процессор и операционная система
                <br>
              </li>
              <li><strong>ОС:</strong> Windows 10 (x64) - Windows 11 (x64)
                <br>
              </li>
              <li><strong>Процессор:</strong> Intel Core i5-2310
                <br>
              </li>
              <li><strong>Оперативная память:</strong> 6 GB ОЗУ
                <br>
              </li>
              <li><strong>Видеокарта:</strong> Geforce GTS 450 or equivalent or better
                <br>
              </li>
              <li><strong>DirectX:</strong> версии 11
                <br>
              </li>
              <li><strong>Сеть:</strong> Широкополосное подключение к интернету
                <br>
              </li>
              <li><strong>Место на диске:</strong> 2 GB</li>
            </ul>
          </ul>
        </div>
          <ul>
            <strong>Рекомендованные:</strong>
            <br>
            <ul class="bb_ul">
              <li>64-разрядные процессор и операционная система
                <br>
              </li>
              <li><strong>ОС:</strong> Windows 10 (x64) - Windows 11 (x64)
                <br>
              </li>
              <li><strong>Процессор:</strong> Intel Core i5 4th Generation and above
                <br>
              </li>
              <li><strong>Оперативная память:</strong> 8 GB ОЗУ
                <br>
              </li>
              <li><strong>Видеокарта:</strong> Geforce GTX 600 Series or equivalent or better
                <br>
              </li>
              <li><strong>DirectX:</strong> версии 11
                <br>
              </li>
              <li><strong>Сеть:</strong> Широкополосное подключение к интернету
                <br>
              </li>
              <li><strong>Место на диске:</strong> 2 GB</li>
            </ul>
          </ul>`,
        'categories': [
          {
            'name': "shooter",
            'link': ""
          },
          {
            'name': "shooter",
            'link': ""
          },
          {
            'name': "shooter",
            'link': ""
          },
        ],
        'status': "new",
    },
    {
        'id': 1,
        'name': "Valorant",
        'link': "",
        'imgH': "img/banners/1.jpg",
        'imgW': "img/banners/a1.jpg",
        'treilerSrc': "video/2.MP4",
        'bigBanner': "img/banners/d5770738dde027118563ae96ae58e7d3.jpg",
        'newPrice': 500,
        'oldPrice': 799,
    },
    {
        'id': 2,
        'name': "Counter Strike 2",
        'link': "",
        'imgH': "img/banners/2.jpg",
        'imgW': "img/banners/a2.jpg",
        'treilerSrc': "video/3.MP4",
        'bigBanner': "img/banners/52nahuy.jpg",
        'newPrice': 679,
        'oldPrice': 800,
    },
    {
        'id': 3,
        'name': "Player Unknowns Buttle Grounds",
        'link': "",
        'imgH': "img/banners/3.jpg",
        'imgW': "img/banners/a3.jpg",
        'treilerSrc': "video/4.MP4",
        'bigBanner': "img/banners/image 10.png",
        'newPrice': 120,
        'oldPrice': 350,
    },
    {
        'id': 4,
        'name': "Ghosts",
        'link': "",
        'imgH': "img/banners/4.jpg",
        'imgW': "img/banners/a4.jpg",
        'treilerSrc': "video/2.MP4",
        'bigBanner': "",
        'newPrice': 1255,
        'oldPrice': 1600,
    },
    {
        'id': 5,
        'name': "Drill Aviasion",
        'link': "",
        'imgH': "img/banners/5.jpg",
        'imgW': "img/banners/a5.jpg",
        'treilerSrc': "video/4.MP4",
        'bigBanner': "",
        'newPrice': 3678,
        'oldPrice': 4000,
    },
    {
        'id': 6,
        'name': "ALBLAK 52",
        'link': "",
        'imgH': "img/banners/6.jpg",
        'imgW': "img/banners/a6.jpg",
        'treilerSrc': "video/3.MP4",
        'bigBanner': "",
        'newPrice': 7952,
        'oldPrice': 8812,
    },
    {
        'id': 7,
        'name': "FRIENDLY THUG 52 NGG",
        'link': "",
        'imgH': "img/banners/7.jpg",
        'imgW': "img/banners/a7.jpg",
        'treilerSrc': "video/IMG_6148.MP4",
        'bigBanner': "",
        'newPrice': 812,
        'oldPrice': 1812,
    },
    {
        'id': 8,
        'name': "YA USTAL PISAT",
        'link': "",
        'imgH': "img/banners/8.jpg",
        'imgW': "img/banners/a8.jpg",
        'treilerSrc': "video/IMG_6148.MP4",
        'bigBanner': "",
        'newPrice': 4599,
        'oldPrice': 5120,
    },
    {
        'id': 9,
        'name': "Call Of Duty: Cold War 2",
        'link': "",
        'imgH': "img/banners/9.jpg",
        'imgW': "img/banners/a9.jpg",
        'treilerSrc': "video/IMG_6148.MP4",
        'bigBanner': "",
        'newPrice': 6700,
        'oldPrice': 7000,
    },
    {
        'id': 10,
        'name': "Game Of Mir",
        'link': "",
        'imgH': "img/banners/10.jpg",
        'imgW': "img/banners/a10.jpg",
        'treilerSrc': "video/IMG_6148.MP4",
        'bigBanner': "",
        'newPrice': 4355,
        'oldPrice': 4500,
    },
    {
        'id': 11,
        'name': "On The Run 2",
        'link': "",
        'imgH': "img/banners/11.jpg",
        'imgW': "img/banners/a11.jpg",
        'treilerSrc': "video/IMG_6148.MP4",
        'bigBanner': "",
        'newPrice': 2344,
        'oldPrice': 3050,
    },
]

let basketProducts = [
    {
        'id': 0,
        'name': "Call Of Duty: Cold War 2",
        'link': "gameCard.html",
        'steamLink': "",
        'imgH': "img/banners/farmsim.png",
        'imgW': "img/banners/img1.png",
        'newPrice': 4599,
        'oldPrice': 5120,
    },
    {
        'id': 5,
        'name': "Drill Aviasion",
        'link': "",
        'imgH': "img/banners/5.jpg",
        'imgW': "img/banners/a5.jpg",
        'steamLink': "",
        'newPrice': 3678,
        'oldPrice': 4000,
    },
    {
        'id': 2,
        'name': "Counter Strike 2",
        'link': "",
        'steamLink': "",
        'imgH': "img/banners/2.jpg",
        'imgW': "img/banners/a2.jpg",
        'newPrice': 679,
        'oldPrice': 800,
    },
    {
        'id': 4,
        'name': "Ghosts",
        'link': "",
        'imgH': "img/banners/4.jpg",
        'imgW': "img/banners/a4.jpg",
        'steamLink': "",
        'newPrice': 1255,
        'oldPrice': 1600,
    },
    {
        'id': 6,
        'name': "ALBLAK 52",
        'link': "",
        'imgH': "img/banners/6.jpg",
        'imgW': "img/banners/a6.jpg",
        'steamLink': "",
        'newPrice': 7952,
        'oldPrice': 8812,
    },
    {
        'id': 7,
        'name': "FRIENDLY THUG 52 NGG",
        'link': "",
        'imgH': "img/banners/7.jpg",
        'imgW': "img/banners/a7.jpg",
        'steamLink': "",
        'newPrice': 812,
        'oldPrice': 1812,
    },
]